...
#That command above printed out unigrams AND bigrams.

#Alright, let's try to tokenize into CHAPTERS and PARAGRAPHS instead of words now.
#We will use Jane Austen's Northanger Abbey.
...
#Now we have 1020 seperate paragraphs we can analyzed. 

#We can also turn the_fir_tree from a "one line per element" to "one
...
#We can also maintain all the fairy tales as part of their own thing
...
#2.4 Building your own tokenizer (finally!)

#Example 1: tokenize to characters, only keeping letters
letter_tokens <- str_extract_all(string = "This sentence include 2 numbers and 1 period.",
  pattern = "[:alpha:]{1}"
)
letter_tokens
