...
stopword_rec <- function(stopword_name) {
  ...
  #This is another step of preprcessing, so again the textbook decides
  #to class this as part of preprocessing recipe. Again we have to build
  #a small recipe wrapper helper function to keep functions all organized
  
  #Later we extract the unigrams, and use linear SVN specification
  #Lastly we will predict the YEAR of a SPECIFIC SC OPINION
  #with 1gram, 1 and 2gram, 1 and 2 and 3gram
  ...
  ngram_rec(list(n = 3, n_min = 1)) #that gives us ngrams from 1 2 and 3
  ...
  #test model's performance with various random seeds, as usual
  ...
  list(`1` = unigram_rs,
    ...
    labs(
      ...
      subtitle = "For the same number of tokens, 1grams performed best"
    )
  
  #6.6 lemmatization models -------------------------------------------------
  
  spacyr::spacy_initialize(entity = FALSE)\ #this takes an actual minute to run
 ...
 small_hash(stri_trans_nfc(s1)
       ...
       #starting to look more normalized, yep     
            s2 <- "scho\U0308n"
            s1 == s2 #ain't true
  #6.8 What evaluation metics are
            ...
            final_rs %>%
              ...
              labs(
                x = "Number of tokens",
                title = "Linear SVM performance across # of tokens",
...
scotus_fit %>%
  ...
  labs(
    ...
    subtitle = paste("These features are the most importance","in predicting the yr of an opinion")
  )
final_fitted %>%
  collect_predictions() %>%
